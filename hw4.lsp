...
(defun at-least-one-color (n c k)
    (cond ((> c k) nil) ; no remaining colors
          (t       (cons (node2var n c k) (at-least-one-color n (+ c 1) k))) ; node n is color c or must be at least one of the rest
    )
  )
...
; DONE
; every pair of colors needs to be forbidden.
; !(c ^ c+1) ^ !(c ^ c+2) ...
; (!c v !c+1) ^ (!c v !c+2) ...
...
; same as above except with a helper arg to keep track of which combinations of c and another color have been banned
; combinations of c and all colors below ci have been banned
(defun amoc-help (n c ci k)
    (cond ((> c k)  nil)
          ((> ci k) (amoc-help n (+ c 1) (+ c 2) k)) ; all combinations of c and another have been banned, move on to next c
          (t        (cons (list (- (node2var n c k)) (- (node2var n ci k))) ; either n is not c-colored or not ci-colored
                          (amoc-help n c (+ ci 1) k))) ; move on to next pair with c in it
    )
  )
...
(defun generate-edge-clauses (e k)
    (gec-help (first e) (second e) 1 k)
  )

; returns a list of clauses to ensure that nodes x and y cannot have the same color from the set {c,c+1,...,k}
; !((x is c) and (y is c))
; (!(x is c) or !(y is c))
(defun gec-help (x y c k)
    (cond ((> c k) nil)
          (t       (cons (list (- (node2var x c k)) (- (node2var y c k)))
                         (gec-help x y (+ c 1) k)))
    )
  )
